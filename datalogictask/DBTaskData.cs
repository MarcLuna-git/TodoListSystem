...
namespace ToDoListProcess.DL
{
    ...
    {
        // It's my Database connection po
        private readonly string connectionString = "Data Source=DESKTOP-53PHH4Q;Initial Catalog=DbTaskData; Integrated Security=True;TrustServerCertificate=True;";
        ...
        public bool MarkAsDone(int index, string user)
        {
            ...
            if (index >= 0 && index < tasks.Count)
            {
                ...
                if (!task.Task.StartsWith("[√] "))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@Task", markedTask);
                        cmd.Parameters.AddWithValue("@User", user);
                        cmd.Parameters.AddWithValue("@DateAndTime", task.DateAndTime);
                        conn.Open();
